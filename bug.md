...
- **[P2 — awaiting user approval to apply] RPC EXECUTE grants open to anon/authenticated** — found in the 2026-06-10 post-apply grant audit: Supabase default privileges give anon+authenticated EXECUTE on every new public function (`REVOKE FROM PUBLIC` doesn't strip role-specific grants). Affected: `finish_club_match`, `delete_club_match`, `create_club_locked_pair` (PUBLIC too), `remove_club_player_and_promote`, `start_match_atomic` + new `apply_group_team_delta`, `add_club_player`, `bump_session_version`. (`record_match_score`/`reorder_tournament_queue`/`swap_pending_match_numbers` already service_role-only.) **Not exploitable today** — all touched tables verified RLS-on with SELECT-only policies → anon-invoked SECURITY INVOKER writes affect 0 rows or error. Hardening migration ready at `supabase/migrations/20260610000400_revoke_rpc_execute_anon.sql` (signatures verified against live pg_proc) — apply + commit on user approval.
...
- **Bundled**: dead `team_players.level` text refs removed (`types.ts`, print roster, `csv.ts` → `embeddedReal` via `levels:level_id(real)` embed) — code-side prerequisite for M4 `DROP COLUMN level` (still pending: post-master-deploy + Gate-4 confirm).
- **NEW FINDING (open, awaiting approval)**: post-apply grant audit — 4 older RPCs (`finish_club_match`, `delete_club_match`, `create_club_locked_pair`, `remove_club_player_and_promote`, `start_match_atomic`) + the 3 new ones carry anon/authenticated EXECUTE from Supabase default privileges (`REVOKE FROM PUBLIC` doesn't strip them). **Not exploitable** — all touched tables verified RLS-on with SELECT-only policies, so anon-invoked SECURITY INVOKER writes hit 0 rows/error. Hardening migration written (`20260610000400_revoke_rpc_execute_anon.sql`, NOT applied/committed yet) — see Open.
