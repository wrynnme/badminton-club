...
- **Deferred — ต้อง DB migration (ตาม preference: P2 ที่ต้องแตะ prod DB → conservative ไม่แตะ):** `remove_club_player_and_promote` ไม่ lock clubs row เหมือน add (cap overshoot ตอน interleave หายาก) · `clone_global_levels_to_club` DEFINER ไม่มี ownership check ภายใน (ปัจจุบันปลอดภัย — EXECUTE service-role-only)
