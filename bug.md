...
- ~~**[P2 ×11 remaining]** correctness/concurrency/hardening leads~~ **5 more closed 2026-06-10** (re-verified vs current code — `importPairsCsv` whole-table-scan part was already fixed by the team-scoped `.in("team_id", allTeamIds)` query). See Resolved entry "2026-06-10 — P2 code-only batch". **Remaining = 3 that need a prod migration / infra (deferred per the conservative P2+migration preference):**
  - **[P2 — deferred, needs infra]** `route.ts:19` unbounded guest-profile creation (no rate limit) — a proper fix needs IP capture + a throttle store/table (migration) or external rate-limiter; in-memory is per-instance/weak on serverless. Flagged for a decision.
  - **[P2 — evaluated 2026-06-10, deferred: zero prod consumers]** `classes.ts:544` `getNextGlobalMatchNumber` read-then-insert race → two concurrent class generations collide `match_number` (display-ordering only, no data corruption). Fix needs a verbose per-tournament advisory-lock insert RPC + rewiring 3 generate actions + preserving pre-assigned knockout ids. **Competition mode (classes) has NO prod consumers** (no competition tournament exists) → cost ≫ benefit; revisit if/when a real multi-class event is run.
