...
- ~~**[P2 ×11 remaining]** correctness/concurrency/hardening leads~~ **5 more closed 2026-06-10** (re-verified vs current code — `importPairsCsv` whole-table-scan part was already fixed by the team-scoped `.in("team_id", allTeamIds)` query). See Resolved entry "2026-06-10 — P2 code-only batch". **Remaining = 3 that need a prod migration / infra (deferred per the conservative P2+migration preference):**
  ...
  - **[P2 — deferred, needs migration]** `classes.ts:544` `getNextGlobalMatchNumber` read-then-insert race → two concurrent class generations can collide `match_number` (display-ordering only; no unique constraint). Fix = advisory-lock RPC or unique index.
  - **[P2 — deferred, needs migration]** `clubs.ts:819` same idle player drafted into two matches by concurrent `buildNextClubMatchAction` (busy-set read + insert not atomic). Fix = `pg_advisory_xact_lock` per club. Rare (needs 2 concurrent builders).
