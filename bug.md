...
Static green: `tsc --noEmit` clean · vitest **421/421** · prod `next build` OK. Opt-in `queue_payload_sync` (default false, no migration): match-queue patches individual rows from postgres_changes UPDATE payloads instead of full refetch; INSERT/DELETE → router.refresh; `suppressPatchRef` pauses patches during drag/reorder. Page-level debounced refresh untouched (authority) → purely additive, default-off → cannot regress the working path. **⚠️ Entire realtime-patch path UNEXERCISED at runtime** — the render smoke ran with the flag default-OFF, so it only confirmed the page renders on the *unchanged* path; the subscription, row-patch, and drag-gate never executed even once (not even a single-client happy path). Concurrency (multi-court races, optimistic-vs-payload reconciliation, dnd-vs-realtime) is additionally unverifiable single-client. Needs a live test with the flag ON — first to confirm the basic path even fires, then multi-client for the races — before being recommended for real use; ships off so the running tournament is unaffected.
