...
import {
  createGlobalLevelAction,
  updateGlobalLevelAction,
  deleteGlobalLevelAction,
} from "@/lib/actions/levels";
import type { Level } from "@/lib/types";

// Site-admin editor for the GLOBAL default level set (`levels` where club_id IS
// NULL). Mirrors the per-club ClubLevelsManager, but every mutation targets the
// global set via the *GlobalLevelAction server actions (no clubId).
...
function EditRow({ level, onDone }: { level: Level; onDone: () => void }) {
  ...
  function handleSave() {
    ...
    start(async () => {
      const res = await updateGlobalLevelAction({ id: level.id, real: realNum, label: label.trim() });
      if ("error" in res) {
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      <Input
        ...
        aria-label={t("realAriaLabel")}
      />
      <Input
        value={label}
        onChange={(e) => setLabel(e.target.value)}
        className="h-7 flex-1 text-sm"
        aria-label={t("labelAriaLabel")}
        onKeyDown={(e) => {
...
function LevelRow({ level }: { level: Level }) {
  ...
  function handleDelete() {
    start(async () => {
      const res = await deleteGlobalLevelAction({ id: level.id });
      if ("error" in res) {
        toast.error(res.error);
      } else {
        router.refresh();
    ...
    <div className="flex items-center gap-2 py-1 text-sm">
      ...
      {confirmDelete ? (
        <div className="flex items-center gap-1.5">
          ...
          <Button size="xs" variant="destructive" disabled={pending} onClick={handleDelete}>
            {t("confirmButton")}
          </Button>
          <Button size="xs" variant="ghost" disabled={pending} onClick={() => setConfirmDelete(false)}>
            {t("cancelButton")}
          </Button>
        </div>
      ) : (
        <>
          <Tooltip>
            <TooltipTrigger render={
              <Button size="xs" variant="ghost" className="h-6 w-6 p-0" onClick={() => setEditing(true)}>
...
function AddRow() {
  ...
  function handleAdd() {
    ...
    start(async () => {
      const res = await createGlobalLevelAction({ real: realNum, label: label.trim() });
      if ("error" in res) {
        toast.error(res.error);
      } else {
        setReal("");
...
export function AdminLevelsManager({ levels }: Props) {
  const t = useTranslations("admin.levels");
  return (
    <Card>
      ...
      <CardContent className="space-y-0.5">
        ...
        <AddRow />
        <p className="text-[11px] text-muted-foreground pt-1">{t("deleteWarning")}</p>
      </CardContent>
    </Card>
  );
}
