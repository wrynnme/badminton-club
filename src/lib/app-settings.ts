...
const DEFAULTS: AppSettings = { qr_logo_enabled: true, qr_logo_url: null };
...
export async function getAppSettings(): Promise<AppSettings> {
  ...
  const { data } = await sb
    .from("app_settings")
    .select("qr_logo_enabled, qr_logo_url")
    .eq("id", 1)
    .maybeSingle();
  if (!data) return DEFAULTS;
  return { qr_logo_enabled: data.qr_logo_enabled, qr_logo_url: data.qr_logo_url };
}
