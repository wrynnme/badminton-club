...
11. **Custom ใบเสร็จเรียกเก็บเงิน (club)** — 🔲 **TODO** (2026-06-24, user request) — ให้เจ้าของก๊วน custom ใบเสร็จที่ใช้เรียกเก็บเงินเองได้. ปัจจุบันใบเสร็จเป็น layout ตายตัวใน `club-slip-card.tsx` (`SlipCard`/`buildSlipMeta`) + `PlayerReceipt` ใน `club-payment-collector.tsx` — ดึงจาก `computeClubCostRows` + PromptPay config. **ขอบเขตยังต้อง clarify**: custom อะไรได้บ้าง — หัวบิล/ชื่อก๊วน, ข้อความท้ายบิล (เช่น "ขอบคุณที่มาเล่น"), โลโก้/รูป, เลือกฟิลด์ที่โชว์ (ซ่อนค่าคอร์ท/ลูก/ส่วนลด), สี/ธีม? น่าจะเก็บเป็น `clubs.receipt_template jsonb` (mirror `prize_template`) + editor owner-only ในแท็บค่าใช้จ่าย/ตั้งค่า. ถามผู้ใช้ก่อนเริ่มว่าต้อง custom field ไหนบ้าง.
12. **รับเงินด้วยเลขบัญชีธนาคาร (club)** — 🔲 **TODO** (2026-06-24, user request) — ตอนนี้รับเงินได้แค่ PromptPay: `clubs.promptpay_id` + `src/lib/club/promptpay.ts` (`detectPromptPayType`/`buildPromptPayPayload`) รองรับเฉพาะ **mobile(10)/national_id(13)/ewallet(15)** — เพิ่ม option ให้เจ้าของใส่ **เลขบัญชีธนาคาร** (ธนาคาร + เลขบัญชี + ชื่อบัญชี).
    - **ทางเลือก a (ง่าย/ปลอดภัยสุด):** โชว์ ธนาคาร+เลขบัญชี+ชื่อ เป็น**ข้อความ**บนใบเสร็จ (ไม่มี QR ฝังยอด — payer โอนเอง). fields ใหม่บน `clubs` (`bank_name`/`bank_account_no`/`bank_account_name`) + เลือกวิธีรับเงิน (PromptPay | บัญชีธนาคาร) ในการ์ดตั้งค่าผู้รับเงิน.
