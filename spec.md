...
- **T2 — knockout "best Nth place" bracket fill DONE (2026-06-08, develop)**: new opt-in setting `knockout_fill_byes` (TournamentSettingsSchema, **no migration** — settings jsonb; default **false**). When ON, **team-mode `group_knockout`** fills empty knockout slots with the best non-advancing teams ranked cross-group (e.g. best 3rd-placers) instead of leaving first-round BYEs. Pure helper `selectBracketFillers(rest, need)` in `bracket.ts` (ranks by finishing position → pts → diff → pf; returns ≤ need; no mutation) + 5 vitest cases. Wired in `generateKnockoutAction` (matches.ts team-mode branch): collects non-advancers into `restAdvancers` (rank `advanceCount+1+`), then after the `advancers.length>=2` check pushes `selectBracketFillers(rest, nextPowerOf2(advancers.length) - advancers.length)`. **Gated off the independent-lower-bracket path** (`has_lower_bracket && !allow_drop_to_lower` already consumes the next-rank teams via `slice(advanceCount, advanceCount*2)` → would double-allocate). Settings toggle in `settings-manager.tsx`. **Scope honesty:** prod runs **pair mode only** (1 tournament, pair/group_knockout; Phase 13 competition is pair-only) — pair seeding is division-wide (matches.ts:671), so T2 has **zero effect on any tournament the user currently runs**; it's a team-mode-only feature. tsc clean · vitest **421** · prod build OK. **Not live-tested** — zero team-mode consumers in prod (would need a throwaway seeded team tournament); the fill *logic* is unit-tested, the action wiring is static-verified + default-off.
