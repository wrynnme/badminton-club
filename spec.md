...
- **Slip-verify** `src/lib/club/slip-verify.ts`: `verifySlip(input)` adapter **provider-agnostic** (env `SLIP_VERIFY_PROVIDER` = `easyslip`|`slipok` + `SLIP_VERIFY_API_KEY`; ไม่ตั้ง = `not_configured` → matchSlipToBill ส่ง manual). **implement จริงแล้วทั้ง 2** (image upload → normalize เป็น `SlipVerifyResult`): **EasySlip** `POST developer.easyslip.com/api/v1/verify` Bearer + multipart `file`, อ่าน `data.amount.amount`/`data.receiver.account.name.{th,en}`/`data.transRef`; **SlipOK** `POST api.slipok.com/api/line/apikey/<branchId>` header `x-authorization` + multipart `files` (ต้อง env `SLIP_VERIFY_SLIPOK_BRANCH_ID`), อ่าน `data.amount`/`data.receiver.{displayName,proxy,account}`/`data.transRef`. + pure `matchSlipToBill({detected, billAmount, clubPromptpayId, clubPromptpayName})` → `verified`|`manual` (Hybrid: amount + receiver lenient last-4/name; ไม่ชัวร์→manual). +unit tests.
