...
5. **โหมดระดับฝีมือ** (`skill_level_enabled`) — on/off; เปิด = ใช้ระดับฝีมือใน `level_match`/`smart`
...
**Task backlog — preset ต้องบันทึก "ตั้งค่าผู้รับเงิน" ด้วย (requested 2026-07-05):**
- **Problem:** ตอนนี้ preset เปิดก๊วนใหม่ได้เร็ว แต่ไม่พา payment receiver config ไปด้วย ทำให้เจ้าของก๊วนต้องตั้ง PromptPay/ชื่อผู้รับ/QR/บัญชีธนาคาร/ช่องทางรับเงินซ้ำทุกครั้งหลัง apply preset.
- **Scope:** ขยาย `ClubPresetConfigSchema` ให้เก็บ payment receiver fields ที่ปลอดภัยต่อ owner-owned private preset: `promptpay_id`, `promptpay_name`, `promptpay_qr_image` (URL รูป QR ที่ owner อัปโหลดไว้), และ `receipt_template.payment_show` + `receipt_template.bank` สำหรับบัญชีธนาคาร/ช่องทางใบเสร็จ. ไม่รวม `receipt_logo_url` ใน v1 เว้นแต่ user ขอ เพราะเป็น branding ใบเสร็จมากกว่าผู้รับเงิน.
- **UI:** เพิ่ม section ใน `preset-form.tsx` สำหรับ "ตั้งค่าผู้รับเงิน" โดย mirror fields จากแท็บค่าใช้จ่าย: PromptPay id/name, QR image URL/preview ถ้าใช้ได้, toggle PromptPay/บัญชีธนาคาร, bank name/account no/account name. Validation ต้องเหมือน server action ปัจจุบัน: PromptPay id ต้องผ่าน `isValidPromptPayId`; ถ้าเปิด bank ต้องมี bank name + account no.
- **Apply mapping:** `applyClubPresetAction` ต้องเขียนลง `clubs.promptpay_id`, `clubs.promptpay_name`, `clubs.promptpay_qr_image`, และ `clubs.receipt_template` ตอน insert club row. Preset apply ยังเป็น one-shot seed: แก้ preset ภายหลังไม่กระทบก๊วนที่เปิดไปแล้ว.
- **Security/PII:** payment receiver details ห้าม expose บน public view เหมือนเดิม (`toPublicClub` redacts อยู่แล้ว). Preset actions ต้อง LINE-user-only + owner-only ตามเดิม; อย่า select/return secret slip-verification keys (`club_billing_secrets`) หรือข้อมูลสลิป.
- **Tests:** เพิ่ม unit tests ให้ `parsePresetConfig` recover payment fields แบบ tolerant, action/apply test ยืนยัน club row ที่สร้างมี receiver config ครบ, และ Playwright smoke เปิด preset → apply → แท็บค่าใช้จ่ายเห็นผู้รับเงิน/ช่องทางรับเงินพร้อมใช้งาน.
