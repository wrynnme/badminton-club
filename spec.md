...
- **DB** `clubs`: `court_fee` · `court_split` (`even`|`by_time`) · `shuttle_split` (`even`|`per_match`, CHECK `clubs_shuttle_split_check`) · `shuttle_price` (**ราคา/ลูก** — ใช้ทั้ง 2 mode) · `court_gap_policy` (`spread`|`owner`|`ignore`). `shuttle_fee` = legacy column ค้างไว้ (deprecated, ไม่ใช้ในการคิดค่าลูกแล้ว). `club_players`: `start_time`/`end_time` (time, null = ช่วงก๊วนเต็ม) · `games_played` · `last_finished_at`.
- **Shuttle model (redesign 2026-06-07):** ค่าลูก = ราคา/ลูก × ลูกที่ใช้ (`club_matches.shuttles_used`), ทั้ง 2 mode derive จาก matches (ไม่ใช้คิว → ค่าลูก = 0). **even** (หารเท่า) = `Σ shuttles ทั้งหมด × price ÷ N ทุกคนเท่ากัน`; **per_match** (ตามแมตช์) = `Σ ต่อแมตช์ (shuttles × price ÷ คนในแมตช์)`. by_games เดิม = per_match → merge (migration `20260607000400` drop CHECK เก่า `(even,by_games)` ที่บล็อก per_match [latent bug ของ commit 95488da] → migrate by_games→per_match → re-add CHECK `(even,per_match)`).
...
- **Actions**: `updateClubCostConfigAction` (`CostConfigSchema` shuttle_split `even`|`per_match` + shuttle_price) · `updateClubPlayerSessionAction` (time/games).
- **UI**: `club-cost-manager.tsx` (court fee + gap policy + **ค่าลูก (บาท/ต่อลูก)** = shuttle_price + toggle หารเท่า/ตามแมตช์; shuttle_fee pass-through ซ่อน) · `club-cost-breakdown.tsx` (per-player table; รับ `matches`) · session editor + partial-window label + `hourly-headcount.tsx` ใน `sortable-player-list`.
