...
1. **EXPAND (additive DDL):** `club_series` + `series_members` + `series_partner_pairs` + `clubs.series_id` + `club_players.member_id` — โค้ดเดิมไม่พัง. RPC ใหม่ทุกตัว `REVOKE EXECUTE FROM PUBLIC, anon, authenticated`.
2. **Backfill (assisted):** ตาม decision #5 — preview ก่อน apply เสมอ.
