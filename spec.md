...
7. **i18n TH/EN** — ~~ตัดสินใจ go/no-go ให้ชัด~~ → **PARTIAL ✅ 2026-06-12 (develop) — club components done**. ทุก `.tsx` ใน `src/components/club/` แปลง hardcoded Thai → `useTranslations("club.*")` (next-intl) ครบแล้ว. `messages/th/club.json` เก็บ Thai verbatim; `messages/en/club.json` เก็บ English. tsc 0 errors ใน club components. เหลือ: tournament components, server action error strings, audit log display, LINE notify locale; แผนละเอียด ~10–14 ชม. อยู่ใน UX polish backlog
...
- **i18n ไทย/อังกฤษ** — **PARTIAL ✅ 2026-06-12**: `src/components/club/*.tsx` ครบแล้ว (next-intl `useTranslations("club.*")`; `messages/th/club.json` + `messages/en/club.json` สมบูรณ์; tsc 0; grep ไม่พบ rendered Thai ใน club components). เหลือ:
  - tournament components (`src/components/tournament/`) — ยังเป็น hardcoded Thai ทั้งหมด
  - server action error strings — return error keys แทน Thai literal, resolve ที่ client toast
  - audit log descriptions — เก็บใน DB เป็น Thai (current); translate at display time เมื่อพร้อม
  - LINE notify messages — locale per tournament setting (`settings.notify_locale: "th" | "en"`)
  - locale switcher (TH/EN) ใน SiteHeader — ยังไม่ได้ทำ
  - Date/number formatting → `Intl.DateTimeFormat` + `Intl.NumberFormat` ทุกที่
  - Effort estimate ที่เหลือ: ~10-14 ชม.
