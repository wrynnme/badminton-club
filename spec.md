...
### Group billing via LINE (push เข้ากลุ่มที่ผูกไว้) ✅ CODE DONE 2026-07-12 (v0.31.0)
...
- **ข้อจำกัด LINE ที่กำหนดดีไซน์:** บับเบิลเดียวใส่ทั้ง @mention + รูปไม่ได้ (text = mention ได้แต่ไม่มีรูป; image/flex = มีรูปแต่ mention ไม่ได้ — ยืนยันจาก reference) → ต่อยอด = **1 push, `messages: [text(mentionees), image(QR)]`**. mention สูงสุด 20/ข้อความ (เกินนั้น chunk เป็นหลายข้อความ, clamp 5 msg/push → `overflow` flag); index/length นับ UTF-16 (= JS `.length`, build mention นำหน้า index เริ่ม 0).
...
- **Schema:** migration `20260712000100_club_line_group_bind` — `clubs.line_group_id text` + partial-unique `uniq_clubs_line_group_id ... WHERE line_group_id IS NOT NULL` (1 กลุ่ม/1 ก๊วน). `Club` type +`line_group_id`; `toPublicClub` redact เป็น null (+vitest). **⚠️ ยังไม่ apply prod (รอ user อนุมัติ).**
- **Pure core** `src/lib/club/group-billing.ts`: `bucketBillsByAmount` (จัดกลุ่มตามยอด → reachable/unreachable), `buildGroupBillText` (mention index/length), `buildGroupBillMessages` (chunk 20 + clamp 5 + `overflow`), `buildQrImageMessage`. **8 vitest** reproduce เคสจริง 170/@bee/@pang + 90/@bank/@boy.
- **Helper** `pushMessagesToGroup(groupId, messages)` ใน `line-club.ts` (Push API `to:` รับ group id ตรงๆ).
- **Action** `pushGroupBillsAction({clubId, playerIds?})` ใน `club-billing.ts`: reuse `computeClubCostRows` → resolve `line_user_id` → bucket → gen QR/ยอด (path `{clubId}/group-bill-{amount}.png`) → `pushMessagesToGroup(line_group_id)` → stamp `bill_amount`+`bill_pushed_at` ทุกคนในบัคเก็ตที่ส่งสำเร็จ; audit `group_bills_pushed`. คืน `{amountsPushed, playersTagged, skippedNoLine, overflow}`. keys `actions.club.{noLineGroup,noPayable}`.
- **UI:** ปุ่ม "เรียกเก็บเข้ากลุ่มไลน์" (`club-payment-collector.tsx` — Users icon, disabled เมื่อยังไม่ผูกกลุ่ม/ไม่มีคนจ่าย, tooltip) + section "ผูกกลุ่มไลน์กับก๊วน" (`club-link-controls.tsx` — bound state / คำสั่ง `ผูกก๊วน <token>` + copy / prompt gen token). page ส่ง `lineGroupBound={!!club.line_group_id}`. keys `club.payment.pushGroup*` + `club.linking.bindGroup*` (th/en parity 925=925).
- gates: **tsc 0 · vitest +8 (group-billing) · i18n parity ok**. **ยังไม่ live-test loop จริง** — prereq ก่อนใช้: (1) apply migration, (2) Messaging channel + `LINE_MESSAGING_CHANNEL_ACCESS_TOKEN`/`_SECRET`, (3) เปิด "Allow bot to join group chats" + เชิญบอทเข้ากลุ่ม, (4) LINE Login/Messaging **provider เดียวกัน** (`line_user_id` ตรงกัน ถึงจะแท็กถูกคน), (5) ผู้เล่น link LINE กับก๊วน.
