...
- **Slice 5 — ClassManager UI DONE (2026-06-02, develop)**: `src/components/tournament/class-manager.tsx` — table (code/name/capacity/pairs_per_group/format/advance/match_format) + add/edit Dialog (TanStack Form v1 + zod, format-conditional fields) + delete (confirm + surfaces server guard error) + @dnd-kit reorder (mirrors court-manager debounce/serialized writes). Owner-gated; Tooltip-wrapped actions; SelectValue render-child (Base UI). Wired in `page.tsx` (fetch `tournament_classes` ordered by position; render in Settings tab **only when `tournament.mode === "competition"`**) + `default_match_format` Select added to `settings-manager.tsx`. tsc clean · 350 vitest pass. **Visual verification pending** — no competition-mode tournament exists yet to screenshot (component uses established court-manager/settings-manager/TanStack patterns; flip a tournament to `competition` to render). ScoreForm format-clamp deferred to Slice 6 (needs match→class→format thread).
