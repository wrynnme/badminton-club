...
- **UI**: `club-cost-manager.tsx` (court fee + gap policy + **ค่าลูก (บาท)** = shuttle_price + toggle หารเท่า/ต่อลูก/ต่อแมตช์) · `club-cost-breakdown.tsx` (per-player table; รับ `matches` + `expenses`; `hasShuttle` gate = `shuttle_price > 0` ทุก mode; `SHUTTLE_SPLIT_LABEL` ตรง UI) · session editor + partial-window label + `hourly-headcount.tsx` ใน `sortable-player-list`.
...
- **Score entry 3 โหมด** ✅ DONE (2026-06-08): finish a club match = กรอก score `a:b` เต็ม (winner derive ฝั่ง server via `deriveWinnerSide`) | กดฝั่งผู้ชนะ | ไม่ลงคะแนน. `CompletedRow` แสดง `score_a:score_b`.
...
- **`buildNextClubMatchAction`** error ละเอียด: `available<needed` → `notEnoughPlayersDetail {needed,available,checkedIn,playing}`; คนพอแต่จัดทีมไม่ได้ → `cannotFormMatchSkillLock`.
- **UI** `club-queue-panel.tsx`: `EditPlayersDialog` (ปุ่ม pencil "แก้ผู้เล่น" บน PendingRow → PlayerSelect + clear-slot) · ปุ่ม "เริ่ม" `disabled` + tooltip "ต้องเลือกผู้เล่นให้ครบก่อนเริ่ม" เมื่อ `!isClubMatchFull` · ManualMatchDialog ผ่อนเช็ค "ครบทุกช่อง" + hint "สร้างค้างไว้ได้".
- **Pure** `isClubMatchFull(m, ppt)` ใน `queue.ts` (5 vitest: doubles 4-ช่อง / singles 2-ช่อง / partial). i18n th+en: 5 actions keys + 10 queuePanel keys (key-check + parity PASS). Gates: tsc 0 · vitest 640 · `next build` OK. **Live Playwright net-zero smoke PASS (prod):** manual 1-คน → DB partial row · "เริ่ม" disabled ตอน partial / enabled ตอนครบ · EditPlayersDialog เติมครบ → start → `in_progress` · build บนคนหมด pool → toast "ผู้เล่นไม่พอ: ต้องการ 4 มีพร้อม 0…" · teardown 0 row.
