...
- Remaining slices: Slice 4 `gameWinner(games, format)` branch (fixed_2/best_of_3/best_of_5) + `default_match_format` setting · Slice 5 `ClassManager` UI · Slice 6 per-class tabs + pair-tab class filter + queue prefix · Slice 7 CSV `class_code` · Slice 8 mode selector + upgrade-to-competition action — not started.
