...
- **Dev tooling — Storybook 10.4.6** (added 2026-06-27): framework `@storybook/nextjs-vite` (Vite builder; peer supports Next ^16) + addons a11y/docs/vitest/mcp/chromatic. Tailwind v4 styles render in stories via `import "../src/app/globals.css"` in `.storybook/preview.tsx` — Vite picks up the existing `@tailwindcss/postcss` config, so **no `@tailwindcss/vite` needed**. `vitest.config.ts` is now a **projects array**: project 1 = the original unit tests (node env, unchanged — 768 pass) · project 2 = `storybook` (browser mode, playwright-chromium, runs story play-fns). `npm test` (`vitest run`) runs **unit only**; the browser project is opt-in via `npx vitest run --project storybook` (3 files/8 tests pass). Scripts `storybook` / `build-storybook`. **First domain stories (branch `feat/storybook-stories`):** scaffold examples removed; `src/stories/` now holds `fixtures.ts` (`makeMatch()` full-Match factory + two sample pair `Competitor`s + `pairCompetitorMap`) and 6 story files — `loading-spinner` (UI, no providers), `bracket-match-card` (pending/completed/empty), `entity-link` (router via `parameters.nextjs.navigation` — unwrapped vs as-link), `tv-match-card` (live/completed/pending, i18n), `match-row` (pending/completed/comfortable), `standings-table` (pair/team/empty). Global decorators in `.storybook/preview.tsx` mirror the app root: `NextIntlClientProvider` (TH, all 11 namespaces imported explicitly) + `TooltipProvider`. Custom `parameters.viewport.options` presets (Phone 390 / Tablet 768 / Desktop 1280 / TV 1920×1080 — mapped to court/public/tv surfaces); viewport is **built into Storybook 10 core** (`storybook/dist/viewport`), no separate addon. `@storybook/addon-mcp` is already installed (devDep + `main.ts`). **6 story files / 24 tests** pass under `npx vitest run --project storybook` (flaky only when the Storybook dev server runs concurrently — Vite optimizeDeps contention; clean run is green). Each story file has `tags: ["autodocs"]` (auto Docs page) + `argTypes` Controls + a `Playground` story. **Render-driven pattern:** the 4 object-prop components (bracket/tv/match-row/standings) use **virtual args** (e.g. `status` select, `unit`/`size` radio, `isOwner`/`played`/`empty` booleans) + a `render` fn that builds the Match/competitors from those args — so state is editable live from the Controls panel. These omit the `component` meta field (virtual args ≠ component prop shape; controls/docs come from `argTypes`); `loading-spinner`/`entity-link` keep `component` (primitive props map 1:1). **Server-action mocking:** `match-row`/`score-form` import the `@/lib/actions/matches` `"use server"` module (supabase service-role — browser-unsafe); `.storybook/main.ts` `viteFinal` aliases it (exact-match regex, prepended) to no-op stubs in `.storybook/mocks/actions-matches.ts`. **`standings-table` refactored (split):** `StandingsTableView` (presentational, takes a `labels` object — sync, storyable) + async `StandingsTable` wrapper that resolves `getTranslations` labels and renders the view; **call sites unchanged** (pair-stage/group-stage/class-group-stage), mirrors the CSV labels pattern. `next build` verified RSC-clean. **Play functions (from `storybook ai setup`, adapted):** one project-wide `CssCheck` story (asserts `getComputedStyle().display === "flex"` → proves Tailwind/`globals.css` loaded in the preview, not just that the component mounted) + smoke `play`s on `entity-link` AsLink (router-derived `href` resolves) and `tv-match-card` Live (i18n `กำลังแข่ง` renders → NextIntl decorator works); `expect` imported from `storybook/test`. The `ai setup` prompt's other suggestions were **rejected as project-mismatched**: it mis-detected **pnpm** (project is npm — would corrupt the lockfile), and recommended MSW + `mockdate` + `src/index.css` + a non-existent `src/contexts/SessionContext` — none apply (components render from props/fixtures; server actions are aliased, not network-mocked). **shadcn CLI bumped 4.11.0 → 4.12.0** in the same task; `.mcp.json` shadcn server pinned to local `node_modules/.bin/shadcn` (was `npx shadcn@latest`, which broke when 4.11.1 shipped a bad `Invalid Version`).
...
โหมดติ๊กเลือกหลายคนใน `sortable-player-list.tsx` (ปุ่ม "เลือกหลายคน"/"เสร็จสิ้น", canManage only; เปิดแล้ว dnd ปิด — `useSortable disabled: !canManage || selectMode` ทั้ง active+reserve, drag handle ซ่อน; per-row Checkbox + select-all แบบ indeterminate) + sticky `BulkActionBar` เมื่อเลือก ≥1: เช็คอิน / ยกเลิกเช็คอิน / ตัวจริง / สำรอง / แก้เวลา+เกม (`BulkSessionDialog` — enable-checkbox ต่อ field ส่งเฉพาะช่องที่ติ๊ก) / ลบ (`BulkDeleteDialog` — รายชื่อ scrollable + impact bullets + ปุ่มแดง; ทั้งสอง dialog มี `max-h-[90dvh] overflow-y-auto`). 4 actions ใหม่ใน `club-players.ts`: `bulkCheckInClubPlayersAction({clubId,playerIds,checkIn})` (idempotent `.is/.not null` mirror tournament pattern, คืน `{ok,count}`) · `bulkSetClubPlayerStatusAction({clubId,playerIds,status})` (admin override ไม่เช็ค cap, `.neq(status)` อัปเดตเฉพาะ row ที่ต่าง, คืน `{ok,count}`) · `bulkUpdateClubPlayerSessionAction({clubId,playerIds,start_time?,end_time?,games_played?})` (patch เฉพาะ field ที่ส่ง, `""`→null, reject 0 fields, คืน `{ok,count}`) · `bulkDeleteClubPlayersAction({clubId,playerIds})` (sequential RPC `remove_club_player_and_promote` ต่อ id — preserve auto-promote semantics, คืน `{ok,deleted,failed}`). ทั้งหมด zod uuid array 1..100 dedupe · `assertCanManageClub` · tenant-scope `.eq("club_id",…).in("id",…)` · `revalidatePath`. i18n: `club.bulkSelect.*` 35 keys + `actions.club.*` 9 keys (th/en parity).
...
- **DB** `clubs`: `court_fee` · `court_split` (`even`|`by_time`) · `shuttle_split` (`even`|`per_match`|`per_player`|`by_time`, CHECK `clubs_shuttle_split_check`) · `shuttle_price` (**ราคา/ลูก** — ใช้ทุก mode) · `shuttle_hourly` (`integer[]` default `'{}'` — จำนวนลูก/ชั่วโมง index ตาม `sessionHourSlots`; ใช้เฉพาะ `shuttle_split=by_time`, migration `20260623000100_club_shuttle_hourly`) · `court_gap_policy` (`spread`|`owner`|`ignore`). `shuttle_fee` = **DROPPED แล้ว** (migration `20260607001200`, introspect-confirmed 2026-06-10). `club_players`: `start_time`/`end_time` (time, null = ช่วงก๊วนเต็ม) · `games_played` · `last_finished_at`.
...
- **Action** `src/lib/actions/club-billing.ts`: `pushClubBillsAction({clubId, playerIds?})` — วน payable+unpaid ที่มี `line_user_id` (ผ่าน `profile_id→profiles.line_user_id`): snapshot `bill_amount`, gen QR PNG (`qrcode.toBuffer`, EC H) → upload public `club-qr` `${clubId}/bill-${playerId}.png` → Flex บิล (หัวเขียว + ยอด + QR image + "ส่งสลิปกลับแชต") → `pushFlexToUser` → stamp `bill_pushed_at`; audit `bills_pushed`. คืน `{pushed, failed, skippedNoLine}`. guest (ไม่มี LINE) → ใช้ปุ่ม "ส่งสลิป" / "ดาวน์โหลดสลิปทั้งหมด" ใน `ClubPaymentCollector` (เดิมเป็น `ClubSlipShare` แยก — รวมเข้า panel เดียวแล้ว 2026-06-21).
...
- **UI**: `club-tabs.tsx` (4 แท็บ URL-synced: ลงชื่อ/เช็คอิน · ล็อคคู่+คิว · ค่าใช้จ่าย · ตั้งค่า [manager-only]) · `club-queue-settings.tsx` (debounce save; **skill-balance sub-section** เปิดเมื่อ skill_level_enabled — max_skill_gap / balance_strictness / balance_locked_pairs) · `club-queue-panel.tsx` (Tabs รอ/กำลัง/จบ; pending **drag-reorder** @dnd-kit [manager]; per-court build + start/cancel/finish-winner + elapsed ticker + **`CourtSelect`** [manager: pending/in_progress court badge → `<Select>` of `clubs.courts` → `setClubMatchCourtAction`; static badge when read-only or ≤1 court; CompletedRow stays static] + `ShuttleCounter` **symmetric outline stepper `[−ลูก] [+ลูก]`** (icons + "ลูก", minus disabled at 0; redesigned 2026-06-09 from faint ghost glyphs) + `DeleteMatchButton` [confirm dialog, in_progress/completed] + `ManualMatchDialog` + **`InlinePlayerSlot`** [pending-row: แก้ผู้เล่นแบบ **inline autocomplete ต่อช่อง** (ไม่มี dialog) — ตัวเลือกกรองผู้เล่นที่อยู่ช่องอื่นของแมตช์เดียวกันออก (กันเลือกซ้ำ); เลือก/ล้างช่อง → persist ทันทีผ่าน `setClubMatchPlayersAction` (optimistic + revert on error)] + **"เริ่ม" disabled + tooltip "ต้องเลือกผู้เล่นให้ครบก่อนเริ่ม" เมื่อ `!isClubMatchFull`**) · `club-locked-pairs.tsx` (gated `players_per_team===2`) · `edit-club-form.tsx` (always-open). migrations `20260606000300` · `20260607000100/000200/000300/000400/000500` · `20260617000100` (partial roster — drop NOT NULL on side_*_player1).
...
- **T5 realtime — ✅ core verified 2026-06-15 (live, net-zero):** seed throwaway team tournament + 1 in_progress match + `realtime_enabled:true` + `queue_payload_sync:true` → เปิด admin queue tab (LIVE badge = subscribed) → UPDATE match→completed ผ่าน SQL (จำลอง client อื่น) → **page1 เปลี่ยนเอง ไม่ reload** (กำลังแข่ง 1→0, จบแล้ว 0→1) ผ่าน realtime subscription. **ยังไม่ครอบ (honest):** adversarial multi-client races — 2 client drag/reorder พร้อมกัน, optimistic-vs-payload conflict, multi-court simultaneous start — propagation พื้นฐานทำงาน แต่ race-hardening ยังไม่ได้ทดสอบ deterministic
...
**Config jsonb shape (zod `ClubPresetConfigSchema` ใน `src/lib/club/preset.ts`):** `venue`, `schedule_day`, `start_time`, `end_time`, `max_players`, `court_fee`, `shuttle_price`, `court_count`, `players_per_team`, `rotation_mode`, `queue_mode`, `co_admin_ids: string[]`, `regulars: {name, profile_id?, start_time?, end_time?}[]`.

**Apply mapping** (club เพิ่งสร้าง = ว่าง ไม่มี race → insert ตรงได้ ไม่ผ่าน `add_club_player` RPC เพื่อรองรับ profile_id): clubs row (name/venue/play_date=today/start/end/max_players/court_fee/shuttle_price/`courts=['1'..court_count]`/`queue_settings={court_count,players_per_team,rotation_mode,queue_mode}`/owner_id=caller) · co_admin_ids → `club_admins` · regulars → `club_players` (position 1..N, status active≤max_players else reserve). Owner-only; gate `isGuest`. Migration `20260611000100_club_presets`.
