...
- **Entity stats drill-down (player / pair / team / division)** — clickable name anywhere in the app → opens stat page showing match history + win/loss/draw aggregates for that entity. 4 entity types:
  - **Player** (`/tournaments/[id]/stats/player/[playerId]` + public `/t/[token]/stats/player/[playerId]`): all matches where player appears (via their pairs in pair mode, or their team in team mode). Cards: total matches / wins / losses / draws / win rate %, points scored vs against, current streak. Section: match list (date · opponent · score · result), partner breakdown (per pair they played in), opponent breakdown (top 5 most-faced).
  - **Pair** (`/tournaments/[id]/stats/pair/[pairId]` + `/t/[token]/stats/pair/[pairId]`): per-pair history. Same metric cards. Section: head-to-head vs each opponent pair, division position, points trajectory chart (cumulative points across matches).
  - **Team** (`/tournaments/[id]/stats/team/[teamId]` + `/t/[token]/stats/team/[teamId]`): aggregate of all pairs in team. Cards: total matches / win rate / points-for-against. Section: top-performing players (sub-tab per player), per-pair breakdown table.
  - **Division** (`/tournaments/[id]/stats/division/[divKey]` + `/t/[token]/stats/division/[divKey]`): scoped standings + match list within the division. Cards: pair count, completed matches, top scorer pair. Section: full standings (re-use `computeStandings` filtered to division pairs), recent matches, head-to-head matrix.
  - **Linking**: every competitor name in match-row / standings / bracket / TV display becomes a clickable link → its stat page. Hover shows mini-summary tooltip (lazy fetch).
  - **Data layer**: extend `src/lib/tournament/scoring.ts` with `computeEntityStats({ entityType, entityId, matches, pairs, players })` returning a `EntityStats` struct (matches[], played, wins, losses, draws, pointsFor, pointsAgainst, streak, headToHead Map). Pure function; testable with vitest.
  - **Public mode**: passes `isOwner=false` — read-only, no edit affordances.
  - **Backlog priority**: useful right after Dashboard tab + Prize summary land. Pair mode covers 80% of value because production tournaments are pair-mode. Implement pair + player first, team + division second.
