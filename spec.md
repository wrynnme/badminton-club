...
7. **i18n TH/EN** — ~~ตัดสินใจ go/no-go ให้ชัด~~ → **COMPLETE ✅ 2026-06-12 (develop)**. next-intl แบบ cookie-based (ไม่มี URL routing, mirror `theme` cookie) ครอบทั้ง app: components + pages + server-action error/toast strings ครบ. locale switcher (TH⇄EN) อยู่ใน account dropdown. รายละเอียดใน UX polish backlog ด้านล่าง
...
- **i18n ไทย/อังกฤษ** — **COMPLETE ✅ 2026-06-12 (develop)**: next-intl แบบ cookie-based (`locale` cookie, ไม่มี URL routing — mirror `theme` pattern). `src/i18n/` (config/request/locale/actions), 10 namespaces (`messages/{th,en}/*.json`): common · nav · home · auth · settings · club · tournament · stats · validation · actions. provider ใน root layout; `useTranslations` (client) / `getTranslations` (server). ทำเป็น batch 1–5b:
  ...
  - **เหลือ (deferred, ไม่บล็อก):** CSV export/template headers (ผูกกับ import parser — แปลแล้วเสี่ยง round-trip), `permissions.ts` throw strings (caught+shown แต่เป็น non-action helper), Date/number → `Intl.*` formatting ทั่ว app
