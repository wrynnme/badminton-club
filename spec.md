...
- **Not built**: auto-rotate-all-courts, time_limit enforcement (hint), Realtime (manual refresh). (`not_ready_action` wired 2026-06-22 — uses check-in as the readiness signal; see A3 below.)
...
>
> **ยังไม่ทำ (A ที่เหลือ):** auto-rotate-all-courts (A1, สร้างทีละสนามด้วยปุ่ม), ~~not_ready_action wiring (A3)~~ → **✅ DONE 2026-06-22** (ใช้เช็คอินเป็นตัวบอกความพร้อม: `skip`=ตัดคนยังไม่เช็คอินออก [default = พฤติกรรมเดิม] / `requeue`=ดึงได้แต่ต่อท้ายคิว ผ่าน `QueuePlayer.notReady` ready-first ordering; มีผลเมื่อมีคนเช็คอินแล้ว ≥1 คน), game_time_limit enforcement (A4, UI hint). ~~Realtime (A5)~~ → **✅ A5 DONE 2026-06-15** (Broadcast-from-DB; ดู roadmap #8 ด้านบน).
>
