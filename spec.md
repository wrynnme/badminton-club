...
- **Entity stats drill-down (player / pair / team / division)** — clickable name anywhere in the app → opens stat page showing match history + win/loss/draw aggregates for that entity. 4 entity types:
  ...
  - **Player** (`/tournaments/[id]/stats/player/[playerId]` + public `/t/[token]/stats/player/[playerId]`): all matches where player appears (via their pairs in pair mode, or their team in team mode). Cards: total matches / wins / losses / draws / win rate %, points scored vs against, current streak. Section: match list (date · opponent · score · result), partner breakdown (per pair they played in), opponent breakdown (top 5 most-faced). **Phase B — not yet built**.
  - **Team** (`/tournaments/[id]/stats/team/[teamId]` + `/t/[token]/stats/team/[teamId]`): aggregate of all pairs in team. Cards: total matches / win rate / points-for-against. Section: top-performing players (sub-tab per player), per-pair breakdown table. **Phase C — not yet built**.
  - **Division** (`/tournaments/[id]/stats/division/[divKey]` + `/t/[token]/stats/division/[divKey]`): scoped standings + match list within the division. Cards: pair count, completed matches, top scorer pair. Section: full standings (re-use `computeStandings` filtered to division pairs), recent matches, head-to-head matrix. **Phase C — not yet built**.
  - **Linking (Phase D)**: every competitor name in match-row / standings / bracket / TV display becomes a clickable link → its stat page. Hover shows mini-summary tooltip (lazy fetch). Not yet built.
