...
**Pure algorithm `buildNextMatch(pool, settings, stayingSide?)`** (`queue.ts`): pool = checked-in & not-currently-playing players. Ordering by `queue_mode`: `fifo` (position→joined_at), `rest_longest`/`level_match` (last_finished_at asc, null=front → games_played → fifo), `level_match` (anchor = most-rested, fill nearest-level). Split into sides: `skill_level_enabled` → greedy level-balance; else pick-order. `winner_stays` → caller passes `stayingSide` (sideA kept, opponents drawn from pool); `winner_stays_max` streak cap enforced in action layer. Returns null when pool < 2×players_per_team. **level_match = rest_longest ordering + level-balanced split** (legacy `smart` folded into `level_match` 2026-07-07 — was byte-identical; `parseQueueSettings`/`parsePresetConfig` translate stored `smart`→`level_match`).
...
4. **โหมดคิว** (`queue_mode`) — `rest_longest` พักนานไปก่อน (default · แนะนำ) | `fifo` เข้าก่อนออกก่อน | `level_match` จับคู่ตามระดับ (+ แบ่งฝั่งสมดุลเมื่อเปิดระดับฝีมือ)
5. **โหมดระดับฝีมือ** (`skill_level_enabled`) — on/off; เปิด = ใช้ระดับฝีมือใน `level_match`
