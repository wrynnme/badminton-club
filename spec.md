...
- **New component** `src/components/tournament/score-matrix.tsx` (`"use client"`).
  ...
  - Score cells: top line `{rowGames}:{colGames}` colored via `RESULT_TEXT_CLASS[result]`; bottom line `{rowPoints}-{colPoints}` in `text-[10px] text-muted-foreground tabular-nums`. `scheduled` → `·`; `none` → empty.
  - Column headers + row headers: color dot + name wrapped in `<EntityLink>` (same entity type as `unit`).
  - Table wrapped in `overflow-x-auto w-full` so wide matrices scroll horizontally without breaking layout.
- **`group-stage.tsx` — GroupCard**: added `const [view, setView] = useState<"list"|"matrix">("list")`. When `showMatches` is expanded, a segmented "ตาราง" / "Matrix" button pair (ghost, size sm) appears top-right of the matches section. Active button = `text-foreground font-medium`; inactive = `text-muted-foreground`. Toggling switches between `<ScoreMatrix unit="team" />` and `<MatchList />`.
- **`pair-stage.tsx` — division cards**: added `const [matrixDivs, setMatrixDivs] = useState<Set<string>>(new Set())` keyed by `String(divKey)`. Inside each division `CollapsibleContent`, a "ตาราง" / "Matrix" button pair (same style) appears above the match list. `getDivisionCompetitors(matchList)` (already in scope) supplies competitors to `<ScoreMatrix unit="pair" />`.
...
- **Round-robin score matrix view** — ✅ DONE 2026-05-27. Toggle "ตาราง"/"Matrix" ข้าง match list ทั้ง GroupCard (`unit="team"`) + PairStage division cards (`unit="pair"`). Component generic `score-matrix.tsx` + pure helper `score-matrix.ts` (`buildScoreMatrix`). Cell จากมุม row: games-won `2:1` + แต้มรวม `42-38` สี W/L/D; `·` = scheduled; ว่าง = no fixture (intra-team ใน pair mode เพราะ `generateAllPairMatches` จับ inter-team เท่านั้น). 24 vitest ใหม่ (269→293). ดู section "Score Matrix View (2026-05-27)" ด้านบน.
