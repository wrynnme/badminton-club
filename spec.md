...
- Remaining slices: Slice 5 `ClassManager` UI + settings-manager `default_match_format` toggle + ScoreForm format clamp · Slice 6 per-class tabs + pair-tab class filter + queue prefix · Slice 7 CSV `class_code` · Slice 8 mode selector + upgrade-to-competition action — not started.
