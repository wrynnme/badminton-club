...
fast-path ให้ผู้เล่น **เชื่อม LINE เองในกลุ่มที่ผูกก๊วนไว้ ควบคู่** manager pool เดิม (ไม่แทนที่). ผู้เล่นแท็กบอท + พิมพ์ `เชื่อมไลน์ <ชื่อในโพย>` → เจอ guest row เดียวชื่อตรง + `profile_id IS NULL` → **auto-link ทันที** + reply ยืนยัน + push; **ทุกเคสไม่สวย** (กำกวม/ถูกจอง/ไม่เจอ) → หย่อนเข้า `club_link_requests` pending ให้ manager (ไม่มี disambiguation ในแชท).
