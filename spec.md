...
- **DB** `clubs`: `court_fee` · `court_split` (`even`|`by_time`) · `shuttle_split` (`even`|`per_match`|`per_player`, CHECK `clubs_shuttle_split_check`) · `shuttle_price` (**ราคา/ลูก** — ใช้ทุก mode) · `court_gap_policy` (`spread`|`owner`|`ignore`). `shuttle_fee` = legacy column ค้างไว้ (deprecated, ไม่ใช้ในการคิดค่าลูกแล้ว). `club_players`: `start_time`/`end_time` (time, null = ช่วงก๊วนเต็ม) · `games_played` · `last_finished_at`.
...
- **UI**: `club-cost-manager.tsx` (court fee + gap policy + **ค่าลูก (บาท/ต่อลูก)** = shuttle_price + toggle หารเท่า/ตามแมตช์/ต่อคน; shuttle_fee pass-through ซ่อน) · `club-cost-breakdown.tsx` (per-player table; รับ `matches` + `expenses`) · session editor + partial-window label + `hourly-headcount.tsx` ใน `sortable-player-list`.
