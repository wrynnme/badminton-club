...
- **ข้อจำกัด (ยอมรับ)**: โหมดผู้ชนะอยู่ต่อ — ช่อง winnerOf เติมตอน finish โดย RPC จึง**วางแผน opponent-variety ของผู้ชนะล่วงหน้าไม่ได้** (ทำได้แค่ challenger). กรณี bench เล็กมาก (เช่น 8 คน = 1 ก๊วนพักพอดี) rest-spacing บังคับให้ foursome คงที่ — variety หมุนได้แค่ "การแบ่งฝั่ง" ภายในก๊วน (8 คน/N3 → 12 partnership ครบ 0 ซ้ำ = optimal แล้ว); pool ใหญ่พอ (≥ 3 ก๊วน) จะผสมข้ามก๊วนได้เต็มที่
- **Tests**: `pair-history.test.ts` (8) + `batch-queue.test.ts` variety block (4: 8/N3 → 12 partnership distinct 0 ซ้ำ · seeded history เลี่ยงคู่เดิม · locked pair ยังอยู่ด้วยกันแม้ history · ทุกคนยัง ≥ N). รวม 806/806
