...
fast-path ให้ผู้เล่น **เชื่อม LINE เองในกลุ่มที่ผูกก๊วนไว้ ควบคู่** manager pool เดิม (ไม่แทนที่). ผู้เล่นพิมพ์ `เชื่อมไลน์ <ชื่อในโพย>` (แท็กบอทหรือไม่ก็ได้ — v0.47.0) → เจอ guest row เดียวชื่อตรง + `profile_id IS NULL` → **auto-link ทันที** + reply ยืนยันในกลุ่ม; **ทุกเคสไม่สวย** (กำกวม/ถูกจอง/ไม่เจอ) → หย่อนเข้า `club_link_requests` pending ให้ manager (insert-when-absent — ไม่ปลุก request ที่ manager dismiss ไว้; ไม่มี disambiguation ในแชท).
- **Webhook** `src/app/api/line/webhook/route.ts`: refactor `after()` loop → dispatcher `handleEvent` → `handleBind` (`ผูกก๊วน` เดิม) | `handleSelfLink`. **keyword 2 ระดับ (v0.47.0, เดิมบังคับ @mention):** แท็กบอท (`message.mention.mentionees[].isSelf === true`) → keyword หลวม (`เชื่อม <ชื่อ>` พอ); **ไม่แท็ก → keyword เข้ม** ต้องมีคำ ไลน์/line (`เชื่อมไลน์ <ชื่อ>`) กัน false-trigger จากแชทปกติที่ขึ้นต้น "เชื่อม…"; bind แยกด้วย `BIND_RE` ก่อน (mutually exclusive). orchestration `resolveSelfLink` อยู่ไฟล์เดียวกัน.
- **Pure core** `src/lib/club/line-self-link.ts`: `parseSelfLinkCommand(text, mentionedSelf, mentionees)` (strip mention ด้วย offset UTF-16 → เลือก regex ตาม `mentionedSelf`: หลวม `^เชื่อม[ไลน์|line]? <name>` / เข้ม `^เชื่อม(ไลน์|line) <name>`; คืน `link|usage|null`), `classifyRosterMatch(rows, name)` → `unique|ambiguous|taken|not_found` (match ทุกแถวก่อนเช็ค `profile_id`), `normalizeRosterName`, `stripMentions`. **21 vitest** (`__tests__/line-self-link.test.ts`).
...
> **🔗 Series-first linking (v0.50.0, grilled 2026-07-16, branch `feat/series-first-linking`):** โจทย์ user — "กลุ่มไลน์เชื่อมกับก๊วนเลย เก็บผู้ที่เคยเชื่อมไว้ที่ก๊วน แล้วค่อยลิงก์กับรอบตีทีหลัง". Decisions: จับคู่ชื่อ = **ทะเบียนสมาชิกเป็นหลัก + guest โพยรอบปัจจุบันเสริม** (member ชนะเมื่อทับกัน, dedupe ผ่าน `member_id`); ลงโพย = กติกาเดิม (ขาประจำ seed ตอนเปิดรอบตี, คนอื่นรอผู้ดูแล). Migration `20260716000200` **APPLIED prod**: `club_link_requests.club_id` nullable + partial unique `(series_id, profile_id) WHERE club_id IS NULL` (insert sessionless ใช้ insert ตรง + tolerate 23505 — ON CONFLICT อ้าง partial index ผ่าน PostgREST ไม่ได้). Pure classifier ใหม่ `src/lib/club/link-target-match.ts` (`classifyLinkTarget` → `member|roster|taken|ambiguous|not_found`, 12 เทส). Webhook `resolveSelfLink`: bound-แต่ไม่มี-session ไม่ตอบ no_club แล้ว — upgrade `series_members.profile_id` in place (+ลิงก์แถวโพยที่ member_id ตรง), audit เฉพาะเมื่อมี session (`club_audit_logs.club_id` NOT NULL). Join page: sessionless ไม่ตัน (member → state `"member"`, ไม่ใช่ member → pending `club_id: null`). `club-link-controls.tsx` keyed ด้วย `seriesId` — actions series-gated ใหม่ 5 ตัวใน club-linking.ts (`generate/revokeSeriesJoinTokenAction`, `unbindSeriesLineGroupAction`, `dismissSeriesLinkRequestAction`, `linkRequestToMemberAction`) ผ่าน `assertCanManageSeries`; gate `linkNoSessionsHint` ลบทิ้ง. ลบ session สุดท้าย → pending requests **detach** (`club_id=NULL`) แทน cascade. actions club-keyed เดิมยังอยู่ (ไม่มีผู้เรียก — เก็บกวาดรอบ simplify). **ต่อยอด (feat/member-edit-extras, grill 2 decisions 2026-07-16):** dialog แก้ไขสมาชิกเพิ่ม (1) **เวลาประจำตัว** `series_members.default_start_time/default_end_time` (migration `20260716000300` APPLIED prod) — เปิดรอบตี seed ลง `club_players.start_time/end_time` (คิว pro-rate + หารเงินตามเวลาอ่านต่อได้เอง); มีผล**เฉพาะรอบใหม่** ไม่แตะโพยที่เปิดอยู่ (2) **ส่วน LINE ครบชุดใน dialog**: สถานะ + ยกเลิกเชื่อม (`unlinkSeriesMemberLineAction` — เคลียร์เฉพาะ registry ไม่แตะ club_players ประวัติ) + picker เชื่อมบัญชีที่รู้จัก (`listSeriesLinkableProfilesAction` consent-safe: เฉพาะ profiles ที่มี request/เคยอยู่ roster ของ series นี้ + `linkSeriesMemberToProfileAction` re-check consent server-side); core upgrade+write-through แชร์กับ linkRequestToMemberAction ผ่าน `linkProfileToNameOnlyMember`.

> **🖥️ Display /clubs + /clubs/mine (v0.49.0, grilled 2026-07-16):** โจทย์ = เข้าถึงรอบตีช้า. (1) `/clubs` มี hero "รอบตีวันนี้ / ที่กำลังจะถึง" บนสุด (`upcoming-session-hero.tsx`) — active/ล่าสุดของทุก series **รวมเฉพาะกิจ** ที่ `play_date ≥ วันนี้` (pin Asia/Bangkok), badge "วันนี้", CTA "เข้ารอบตี" ลิงก์ตรง `/clubs/[sid]/s/[id]`, เรียงวันใกล้ก่อน; วันไม่มีนัด = ไม่มีโซนนี้. (2) `/clubs/mine` เปลี่ยนจาก grid แบนเป็น**กลุ่มต่อก๊วน** (`my-session-groups.tsx`, native `<details open>` พับได้ไม่ใช้ client JS) — แถว = รอบตีเรียงวันล่าสุด + badge รอบตีปัจจุบัน + คน X/N + สนาม; เฉพาะกิจ/legacy no-series รวมบัคเก็ต "เฉพาะกิจ" ท้ายลิสต์ (ลิงก์ legacy ใช้ `/clubs/[id]` ผ่าน dispatcher); query embed `club_series!series_id` (hint บังคับ).

> **🗄️ ย้ายเก็บถาวร → /clubs/archive (v0.53.0, user 2026-07-16):** section "เก็บถาวร" (owner's archived series + กู้คืน) ออกจาก `/clubs` ไปหน้าใหม่ `src/app/(app)/clubs/archive/page.tsx` (fetch owner_id + archived_at IS NOT NULL + format ยกไปทั้งชุด). `/clubs` เหลือลิงก์เล็ก count-gated ("ก๊วนที่เก็บถาวร ({count})" → /clubs/archive) — เปลี่ยน full fetch เดิมเป็น `select("id", {count:'exact', head:true})`. `ArchivedSeriesSection` เพิ่ม prop `defaultOpen` (หน้า archive เปิดกาง; usage อื่นพับ). i18n `series.archivedLink`/`series.archivedEmpty` ใหม่.
>
> **🏁 ปิดรอบ / จบแล้ว (v0.53.0, grilled 2026-07-16, branch `feat/session-close-state`):** done state ของรอบตี — **display-only** (6 decisions): (1) ไม่ล็อกการแก้ไข ผู้ดูแลแก้เงิน/โพย/คิวย้อนหลังได้เหมือนเดิม (2) รอบที่จบ**หายจาก `/clubs`** (hero + รอบตีของฉัน) แต่ `/clubs/mine` + หน้าแรกก๊วนแสดงครบพร้อม badge "จบแล้ว" (3) `active_session_id` **ไม่ถูกเคลียร์** — pointer ย้ายเมื่อเปิดรอบใหม่; แถว/การ์ดที่ done โชว์ "จบแล้ว" แทน "รอบตีปัจจุบัน" (4) ปิดได้ 2 ทาง: ปุ่ม "ปิดรอบ" (owner/co-admin, หัวหน้ารอบตี) + **ออโต้เมื่อ `play_date` เลยวันนี้ (Asia/Bangkok)** — derive ในโค้ด ไม่มี cron (5) "ยกเลิกปิดรอบ" ได้เฉพาะรอบที่กดปิดเอง (ออโต้ตามวันไม่มี override) (6) คำ: ปุ่ม "ปิดรอบ" · badge "จบแล้ว" (lock ใน CONTEXT.md). Migration `20260716000400` **APPLIED prod**: `clubs.closed_at timestamptz` nullable (additive). Helper กลาง `src/lib/club/session-done.ts` — `isSessionDone({play_date, closed_at}, todayBkk)` + `todayBangkok()` ใช้ร่วม `/clubs` (filter+hero+series card) / `/clubs/mine` (badge) / series-home / session-view; actions `closeClubSessionAction`/`reopenClubSessionAction` (assertCanManageClub, audit `session_closed`/`session_reopened`); ปุ่ม `close-session-button.tsx`; `toPublicClub` allowlist เพิ่ม `closed_at` (ไม่ sensitive). ไม่ยิง LINE notify ตอนปิด. ผลข้างเคียงตั้งใจ: `/clubs` ส่วนล่างเหลือเฉพาะวันนี้+อนาคต — ประวัติทั้งหมดอยู่ `/clubs/mine`.
>
> **🧭 Flow Step 1+2 (v0.54.0, UX audit + mockup approved 2026-07-20/21, branch `feat/clubs-flow-step12`):** แก้ 4 จุดจาก UX audit 3-persona (mobile, dev server จริง) — ทั้งหมด UI ล้วน ไม่มี migration/action ใหม่: (1) **join page CTA** — ทุก terminal state ของ `/clubs/join/[token]` (already/member/pending + hลัง submit ใน `club-join-confirm.tsx` ที่รับ prop `sessionHref` ใหม่) มีปุ่ม "เข้าดูรอบตีวันนี้" (เมื่อ linked+มี session) / "ดูก๊วนทั้งหมด" + hint ความคาดหวัง (2) **invite card** ใน session-view — `canManage && players.length===0` โชว์การ์ด "ก๊วนพร้อมแล้ว — ชวนเพื่อน" (แชร์ลิงก์ = SeriesInviteSheet / เพิ่มผู้เล่นเอง `?tab=players` / ผูกกลุ่ม LINE → series settings); **ปุ่ม ปิดรอบ ใน header ซ่อนเมื่อโพยว่าง** (การ์ดชวนคือ primary action; ปุ่มกลับมาเมื่อมีผู้เล่น) (3) **series-home ไม่ redirect เงียบ** — non-manager เห็นหน้าอธิบาย: participant (มี series_members.profile_id หรือ club_players ใน series) เห็นชื่อก๊วน + ปุ่ม "เข้ารอบตีปัจจุบัน" (active ?? latest); stranger เห็นข้อความ generic **ไม่โชว์ชื่อก๊วน** (กัน name leak จาก URL เดา) (4) **`series-invite-sheet.tsx`** ใหม่ — Dialog ลิงก์+คัดลอก+แชร์ LINE (line.me/R/share)+QR inline (react-qr-code)+ลิงก์ไปผูกกลุ่ม; ไม่มีลิงก์ → ปุ่มสร้าง (reuse `generateSeriesJoinTokenAction` — assertCanManageSeries เดิม); trigger "ชวนเพื่อน" อยู่ข้าง "เปิดรอบตี" บน series-home overview. i18n ns ใหม่: `club.inviteCard` / `club.inviteSheet` / `club.seriesViewer` + `club.linking.joinCta*`. ค้าง Step 3-5 (first-run 2 ทาง / เตือนรอบซ้อน / โครง nav) — ดู flow artifact.
>
> **🧭 Flow Step 3 (v0.55.0, 2026-07-21, branch `feat/clubs-flow-step3`):** first-run 2 ทาง — `/clubs` empty state (canCreate) เปลี่ยนจากประโยคเดียวเป็น 2 การ์ด: "ตั้งก๊วนของคุณเอง" (ปุ่มสร้าง) / "ถูกชวนเข้าก๊วน?" (guidance ขอลิงก์จากผู้ดูแล — ไม่มีปุ่มเพราะลิงก์มาทาง LINE); guest ยังเห็น `emptyNoCreate` เดิม; **`page.emptyWithCreate` ยังมีผู้ใช้ที่ /clubs/mine — ห้ามลบ**. `/clubs/new` เพิ่ม explainer ใต้หัวฟอร์ม (`page.newClubExplainer` — บอกว่าได้ก๊วนถาวร+รอบแรกในขั้นตอนเดียว). i18n ns ใหม่ `club.firstRun`. UI ล้วน. **Step 4 (branch `feat/clubs-flow-step4`, รวม v0.55.0):** (1) `SeriesOpenSessionButton` รับ `liveSessionDates[]` (play_date ของรอบไม่ done — series-home ส่งให้) → เลือกวันซ้ำขึ้นคำเตือน `series.openDuplicateWarning` (เตือนอย่างเดียว ไม่ block — 2 สนามคนละเวลาวันเดียวกัน legit) (2) `/clubs` ปุ่มสร้างก๊วนลด prominence เป็น outline เมื่อ `playerOnly` (มี rows แต่ไม่ดูแลอะไรเลย). ค้าง Step 5 (โครง nav — grill แยก).
>
...
8. **Preset: deprecate ทิ้งหลัง P2** (user เลือกแรงกว่า rec) — ถอด UI + actions ทันทีที่ปุ่มเปิดนัด ship; ตาราง `club_presets` **DROPPED prod 2026-07-16** (migration `20260716000100`).
